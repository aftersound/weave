...
final class ServiceBasedActorBindingsConfigProvider extends ClientAndApplicationAwareConfigProvider<ClientHandle, ActorBindingsConfig> {

    ServiceBasedActorBindingsConfigProvider(
            ClientHandle client,
            String namespace,
            String application,
    ...
    protected List<ActorBindingsConfig> getConfigList() {
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                return Arrays.asList(configReader.readValue(content, ActorBindingsConfig[].class));
            } else {
                throw new Exception(
                        String.format(
